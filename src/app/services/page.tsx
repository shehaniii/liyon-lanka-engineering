import { getServices, imageUrl, type WebsiteService } from "@/lib/api";
...
export default async function ServicesPage() {
  ...
  return <main>
    ...
    <section className="max-w-7xl mx-auto py-20 px-6"><div className="grid md:grid-cols-3 gap-8">{services.length ? services.map((service) => <article key={service.id} className="border rounded-xl shadow-md hover:shadow-xl transition overflow-hidden bg-white"><div className="h-56 bg-amber-50">{imageUrl(service.imageUrl) ? <img src={imageUrl(service.imageUrl)} alt={service.title} className="h-full w-full object-cover" /> : null}</div><div className="p-8"><h2 className="text-2xl font-bold text-blue-900">{service.title}</h2><p className="mt-4 text-gray-600 leading-7">{service.description}</p></div></article>) : <p className="text-gray-600">Services will be available soon.</p>}</div></section>
  </main>;
}
